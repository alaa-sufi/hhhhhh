...
export default function ProfileContainer({ tab, ...props }) {
  ...
  const percent = 39;
  const navs = [
    { title: t("personal_information"), href: "/dashboard/profile/personal/profile-personally" },
    ...
    { title: t("bank_account_and_cards"), href: "/dashboard/profile/bank-account/details-of-the-bank-account" },
  ]
  const roots = {
    personal: [
      { title: t('profile_personly'), icon: <ClipboardText size="18" className="text-primary" />, href: "/dashboard/profile/personal/profile-personally", },
      ...
      <div className="p-8 bg-white dark:bg-dark-white rounded-lg md:rounded-xl">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-2 mb-6 ">
            ...
            <h1 className="block text-3xl font-bold text-black dark:text-white">{t("profile_personly")}</h1>
          </div>
        ...
        <div className="relative flex justify-between mb-0 bg-secondary dark:bg-dark-secondary  p-[6px] rounded-xl">
          {navs.map((nav, index) => (
            <Link key={index} href={nav.href}  ><a className={` rounded-xl py-5 z-1 text-center text-lg w-1/4 ${router.pathname.startsWith(nav.href.split("/").splice(0,4).join("/")) ? "text-black dark:text-white bg-white dark:bg-dark-white" : "text-gray-400"}`}><Badge content={true}>{nav.title}</Badge></a></Link>
          ))}
        </div>
      </div>
