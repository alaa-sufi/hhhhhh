...
export default function CardAccount({ type, data , handleDeleteDone }) {
    ...
    const { balance, currency, color, leverage, login, freeMargin, equity } = data
    const handleDelete = (login) => {
        setLoadingDeleteButton(true);
        deleteDemoAccount({
            login: login,
            success: () => { setLoadingDeleteButton(false); setOpenMenu(false); handleDeleteDone(type)},
            error: () => setLoadingDeleteButton(false)
        })
    }
    return (
        <div className={`col-span1 relative ${false && '[filter:drop-shadow(0px_0px_5px_rgba(255,0,0,0.6))]'}`} >
            {fix && <span className="absolute top-0 flex items-center justify-center w-8 h-8 transform translate-x-1/2 bg-white  rounded-bl-lg rounded-br-lg right-1/2 z-1"><Pin fill="inherit" /></span>}
            <div className="bg-secondary dark:bg-dark-secondary rounded-xl  ">
                <div className={`  py-5 px-6 rounded-xl text-white relative bg-color`} style={{ "--color": color }}>
                    ...
                    <span className="block mb-10 tracking-widest3 text-slate-300">{type === "real" ? "كلاسيكية" : 'STANDARD'}</span>
                    ...
                    {openMenu && <div className="absolute text-black dark:text-white bg-white dark:bg-dark-white rounded-lg top-4 rtl:left-4 ltr:right-4 z-2 p-3">
                        ...
                        <button className="flex items-center w-full gap-2 px-3 py-2 mb-1 cursor-pointer hover:bg-primary-200/10" onClick={handleFix}><Pin className="w-5 text-black dark:text-white"  fill="inherit"/>{fix ? t("cancel_account_installation") : t("account_installation")}</button>
                        <button onClick={()=>handleDelete(login)} disabled={loadingDeleteButton} className="flex items-center gap-2 px-3 py-2 mb-1 cursor-pointer hover:bg-red-600 hover:text-white">{loadingDeleteButton ?  <Loader size="xs" /> :<Trash className="w-5 text-inherit" size="25" />}{t("delete_account")}</button>
                    </div>}
