...
export default function ButtonTheme({ color, as, outline, href, size, type, block, disabled, loading, fontSize, ...props }) {
    const classes = `cursor-pointer button  text-center rounded-lg ${fontSize === "md" ? "text-xl" : "text-base"} text-white leading-none ${outline ? 'out-primary hover:bg-primary hover:text-white' : "hover:bg-[#441C63] transition"} ${props.className}  ${block && 'w-full block'} ${size === "xs" ? "py-4 px-6" : "py-6 px-6"} ${disabled ? "bg-[#bebebe] pointer-events-none" : `bg-${color} hover:text-white `}`
    //color : primary ,
